...
# If frame has an alpha channel, convert from RGBA to RGB.
    ...
    if frame.ndim == 2:
        ...
        # If image has a single channel in a 3D array (e.g., shape: HxWx1), squeeze it.
        if frame.shape[-1] == 1:
            gray = np.squeeze(frame, axis=-1)
        else:
            # Otherwise, convert from RGB to grayscale.
            gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
    else:
        # Fallback: try converting assuming RGB input.
        gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
...
# Define reduced Controller Policy Network to minimize parameters
class ControllerPolicy(nn.Module):
    def __init__(self, latent_dim=64, hidden_dim=64, action_dim=3):
    ...
    def forward(self, x):
        ...
        # Output steering [-1, 1], throttle [0, 1], and brake [0, 1]
        steer = torch.tanh(actions[:, 0])  # Steering: [-1, 1]
        throttle = torch.sigmoid(actions[:, 1])  # Throttle: [0, 1]
        brake = torch.sigmoid(actions[:, 2])  # Brake: [0, 1]
...
'''
def evaluate_policy(controller, vae, mdn_rnn, env, n_episodes=5, horizon=1000, render=False):
    ...
    for episode in range(n_episodes):
        ...
        while not done and step < horizon:
            ...
            with torch.no_grad():
                _, current_latent, _ = vae.encode(img_tensor)
                current_latent = current_latent.view(1, -1)  # Ensure shape is [1, latent_dim]
                ...
                # Prepare input for MDN-RNN (latent state + action)
                ...
                # Update hidden state (even though we'll get a real observation next step)
                # This helps maintain continuity in the RNN's hidden state
    ...
    for episode in range(n_episodes):
        ...
        while not done and step < horizon:
            ...
            if img_tensor is None:
                print("preprocess_image returned None!")
            else:
                print("img_tensor shape:", img_tensor.shape)


            with torch.no_grad():
                current_latent, _ = vae.encode(img_tensor)
                current_latent = current_latent.view(1, -1)  # Ensure shape is [1, latent_dim]
...
def set_params(controller, params):
    ...
    for param in controller.parameters():
        ...
        param.data.copy_(torch.tensor(params[idx:idx+flat_size], dtype=torch.float32).view_as(param).to(device))
        idx += flat_size
    return controller
...
def fitness_function(params, controller, vae, mdn_rnn, env, n_episodes=3):
    """Fitness function for CMA-ES. Returns the negative average reward."""
    controller = set_params(controller, params)
    avg_reward, _ = evaluate_policy(controller, vae, mdn_rnn, env, n_episodes=n_episodes)
...
def main():
    ...
    os.makedirs("./trained/policy", exist_ok=True)
    
    # 2. Initialize environment
    env = CarlaEnv(town='Town01')
    
    # 3. Initialize controller policy (REDUCED SIZE)
    controller = ControllerPolicy(hidden_dim=64).to(device)  # Reduced hidden size
    ...
    # Initialize CMA-ES optimizer with diagonal mode to reduce memory usage
    sigma0 = 0.1  # Initial step size
    popsize = 16  # Population size
    
    # Using diagonal CMA-ES to reduce memory requirements
    ...
    for generation in range(1, n_generations + 1):
        ...
        for i, params in enumerate(solutions):
            ...
            fitness = fitness_function(params, controller, vae, mdn_rnn, env)
        ...
        if min_fitness < best_fitness:
            ...
            torch.save(controller.state_dict(), f"./trained/policy/best_policy.pth")
        ...
        if generation % 5 == 0:
            # Save controller
            torch.save(controller.state_dict(), f"./trained/policy/policy_gen_{generation}.pth")
            ...
            plt.savefig(f"./trained/policy/rewards_gen_{generation}.png")
            plt.close()
            
            # Save rewards data
            np.save(f"./trained/policy/rewards_gen_{generation}.npy", np.array(generation_rewards))
            
    # Final evaluation with more episodes
    controller = set_params(controller, best_params)
    final_avg_reward, final_rewards = evaluate_policy(
        controller, vae, mdn_rnn, env, n_episodes=10, render=True
    )
    ...
    torch.save(controller.state_dict(), "./trained/policy/final_policy.pth")
    ...
    plt.savefig("./trained/policy/final_rewards.png")
    plt.close()
